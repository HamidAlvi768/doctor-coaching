    ...
    <div id="answermodalparent" class="modal" tabindex="-1">
        <div class="modal-dialog modal-fullscreen">
            <div class="modal-content">
                ...
                <div class="" style="position: absolute;top:70px;left:20px;z-index:999;"><sup style="font-size:0.8rem;">Attempted <strong id="show-attempted">0</strong></sup></div>
                <div class="modal-body" id="answermodal">
                    ...
                    <section id="multipleanswer" style="display:none;">
                        ...
                        <div class="email-overlay">
                            ...
                            <p>CNIC: <?= Yii::$app->user->identity->cnic ?></p>
                        </div>
                    </section>
                ...
                <div class="modal-footer">
                    <div class="left-btns mobile-left">
                    ...
                    <div class="right-btns mobile-right">
...
<style>
    ...
    .modal-footer {
        display: flex;
        justify-content: space-between;
        /* Ensures buttons are on left and right */
        align-items: center;
        padding: 10px 15px;
        flex-wrap: wrap;
        width: 100%;
    ...
    /* Fix left and right button groups */
    .left-btns,
    .right-btns {
        display: flex;
        gap: 10px;
    ...
    .modal-footer {
        display: flex;
        justify-content: space-between;
        /* Ensures left and right buttons stay apart */
        align-items: center;
        padding: 10px 15px;
        flex-wrap: wrap;
        width: 100%;
...
<?php
$allQuestionsUrl = \yii\helpers\Url::to(['question/all-questions']);
...
$quizezUrl = \yii\helpers\Url::to(['site/quizez']);
...
$this->registerJs("
    var allQuestionUrl = '{$allQuestionsUrl}';
    var quizList = '{$quizList}';
    var quizUrl = '{$quizezUrl}';
    var url = '{$url}';
    var url2 = '{$url2}';
    var url3 = '{$url3}';
    ...
    $(document).ready(function() {
        $('#answermodalparent').on('show.bs.modal', function () {
            $('body').addClass('modal-open');
        });

        $('#answermodalparent').on('hide.bs.modal', function () {
    ...
    function showQuestion(id, qn) {
        const q=findObjectByKey(allQuestions, 'qnumber', qn);
        currentQuestion = q;
        getquestion(q);
        $('#answermodalparent').addClass('show');
        document.querySelector('#show-attempted').textContent = attemptedQuiz.length + ' / ' + totalQuestions;
    }

    async function getAllQuestions(){
        fetch(allQuestionUrl + '?quiz-id=' + id + '&session=' + session)
            .then(response => response.json())
            .then(data => {
                if(data){
                    allQuestions = data.questions;
                    attemptedQuiz=data.attempted;
                    showQuestion(id, qn);
                } else {
    ...
    function closeQuiz() {
        if (confirm('Want to close this quiz? ')) {
            if(window.location.href != quizList){
    ...
    function showAnswerResult(ans,feedbackEl) {
        const answers=currentQuestion.answers;
        const qAnswer=findObjectByKey(answers, 'id', ans);
        const isCorrect = qAnswer.is_correct == 1;
    ...
    function submitAnswer() {
        ...
        $.post(submitQuestionUrl, { 
            ...
            if (response.success) {
                attemptedQuiz.push(response.attempted);
                showAnswerResult(parseInt(selectedAnswer),feedbackEl);
                document.querySelector('#show-attempted').textContent = (attemptedQuiz.length)+ ' / ' + totalQuestions;
                $('#submit-btn').hide();
                $('#skip-btn').hide();
                ...
                if(attemptedQuiz.length == totalQuestions) {
                    $('#skip-btn').hide();
                    $('#next-btn').hide();
    ...
    function getquestion(response) {
        ...
        if (response) {
            ...
            $('#qnumber').text(qn +' / '+totalQuestions);
            $('#answermodal #qtext').text(response.question_text);
            var questionType = response.type;
            ...
            if (questionType === 'mcq') {
                ...
                const attempted = findObjectByKey(attemptedQuiz, 'question_id', response.id);
                if (attempted) {
                    $('#skip-btn').hide();
                    $('#submit-btn').hide();
                    $('#next-btn').show();
                    const qAnswer = findObjectByKey(answers, 'id', parseInt(attempted.answer_id));
                    showAnswerResult(qAnswer.id,$('#mcq-feedback'));
                } else {
                    $('#skip-btn').show();
                    $('#submit-btn').show();
                    $('#next-btn').hide();
                ...
                const tfAnswer = response.attempted;
                if (tfAnswer) {
                    $('#truefalsedropdown').hide();
                    $('#skip-btn').hide();
                    $('#submit-btn').hide();
                    $('#next-btn').show();
                    $('#tf-feedback').html(response.result === 'Correct' ? '<i class=\"fas fa-check\"></i> Correct!' : '<i class=\"fas fa-times\"></i> Incorrect!');
                    $('#tf-feedback').removeClass('correct wrong').addClass(response.data.result === 'Correct' ? 'correct' : 'wrong');
                    $('#tf-feedback').show();
                }
            } else if (questionType === 'text') {
                $('#textanswer').show();
                const textAnswer = response.attempted;
                if (textAnswer) {
                    $('#textanswerinput').hide();
                    $('#skip-btn').hide();
                    $('#submit-btn').hide();
                    $('#next-btn').show();
                    $('#text-feedback').html(response.result === 'Correct' ? '<i class=\"fas fa-check\"></i> Correct!' : '<i class=\"fas fa-times\"></i> Incorrect!');
                    $('#text-feedback').removeClass('correct wrong').addClass(response.result === 'Correct' ? 'correct' : 'wrong');
                    $('#text-feedback').show();
                }
            }
            if (time <= 0) {
