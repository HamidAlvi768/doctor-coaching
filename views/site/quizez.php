    ...
    $js .= "
            ...
            function updateRemainingTime_{$data['quiz']->id}() {
                        if (remainingTimeInSeconds_{$data['quiz']->id} > 0) {
                            ...
                            const startBtn = document.getElementById('start-btn-{$data['quiz']->id}');
                            if (startBtn) startBtn.remove();
                            
                            const reattemptBtn = document.getElementById('reattempt-btn-{$data['quiz']->id}');
                            if (reattemptBtn) reattemptBtn.remove();
                            
                            const remainingTime = document.getElementById('remaining-time-{$data['quiz']->id}');
                            if (remainingTime) {
                                remainingTime.textContent = 'Time Up';
                                remainingTime.className = 'time-up';
                            }
                        }
                    }
    ";
